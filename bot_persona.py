...
st.title("🌸 Shion AI - Virtual Assistant 24/7 (3-Tier Groq Engine)")
...
GROQ_API_KEY = (
    ...
    or os.getenv("GROQ_API_KEY") import os
import re
import asyncio
import discord
from discord import app_commands
...
MODEL_RINGAN = "llama-3.1-8b-instant"      # Untuk Chat harian, mention, reply (Limit 14.400 RPD)
MODEL_BERAT = "llama-3.3-70b-versatile"     # Untuk Curhat/Roleplay mendalam (Hemat Limit 500 RPD)
...
def tanya_groq(prompt_text, model_tujuan=MODEL_RINGAN):
    """Fungsi 3-Tier Groq Routing dengan Auto-Fallback untuk Shion."""
    daftar_model = [model_tujuan]
    if model_tujuan != MODEL_RINGAN:
        daftar_model.append(MODEL_RINGAN)  # Fallback ke 8B jika 70B rate limit/error
    ...
    for model_name in daftar_model:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                print(f"⚠️ Groq Shion ({model_name}) Rate Limit 429, mencoba fallback...")
            else:
                print(f"⚠️ Groq Shion ({model_name}) error [{res.status_code}]: {res.text}")
        except Exception as e:
...
async def on_message(message):
    ...
    if is_reply_to_bot or is_mentioned:
        async with message.channel.typing():
            ...
            # Mention/Reply biasa pakai MODEL_RINGAN (8B) agar hemat kuota!
            jawaban = await asyncio.to_thread(tanya_groq, conversation_prompt, MODEL_RINGAN)
            await kirim_pesan_panjang(message, jawaban, mode="reply")
...
@app_commands.describe(
    pesan="Pesan atau pertanyaan kamu untuk Shion",
    mode="Pilih jenis respon (Cepat Santai / Dalam Detail)"
)
@app_commands.choices(mode=[
    app_commands.Choice(name="⚡ Santai & Cepat (Llama 8B Instant)", value="cepat"),
    app_commands.Choice(name="🧠 Detail & Mendalam (Llama 70B Versatile)", value="dalam")
])
async def slash_chat(
    interaction: discord.Interaction, 
    pesan: str, 
    ...
    # Pilih 70B hanya jika user memilih mode "dalam"
    pilihan_model = MODEL_BERAT if (mode and mode.value == "dalam") else MODEL_RINGAN
    prompt_text = f"User [{sender_name}]: {pesan}"
...
async def slash_ping(interaction: discord.Interaction):
    latency = round(bot.latency * 1000)
    await interaction.response.send_message(f"🏓 **Pong!** Shion aktif dengan latency: `{latency}ms`~ Hehe 🌸 (Groq Engine Active)")
